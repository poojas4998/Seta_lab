...
res_ordered
res1 <- res_ordered %>% dplyr::filter(padj < 0.05)
res1
...
write.csv(results_with_gene_column,"KO_control_AngII.csv" )
```
...
#chemical_genetic_pertubation <- "m2.cgp.v2023.1.Mm.symbols.gmt" 
...
chemical_genetic_pertubation <- "m2.cgp.v2023.1.Mm.symbols.gmt" # latest addition
...
```{r}
#chemical_genetic_pertubation gene sets  

chemical_genetic_pertubation_results <- GSEA_function(dataset_path = file.path(path,chemical_genetic_pertubation),namedlist = signif(metric,digits = 10),padj_filter = 0.05,title = "Chemical Genetic Pertubation")


chemical_genetic_pertubation_results$filteredplot

datatable(chemical_genetic_pertubation_results$significant_results, extensions = 'Buttons', options = list(pageLength = 10, autoWidth = TRUE,dom = 'Bfrtip', scrollX = TRUE,buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))

chemical_genetic_pertubation_results$plot
chemical_genetic_pertubation_results$top_10_highest_NES
chemical_genetic_pertubation_results$top_10_lowest_NES

datatable(chemical_genetic_pertubation_results$top_10_highest_NES, extensions = 'Buttons', options = list(pageLength = 10, autoWidth = TRUE,dom = 'Bfrtip', scrollX = TRUE,buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))

datatable(chemical_genetic_pertubation_results$top_10_lowest_NES, extensions = 'Buttons', options = list(pageLength = 10, autoWidth = TRUE,dom = 'Bfrtip', scrollX = TRUE,buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
```
...
write.csv(as.data.frame(canonical_pathway_results$significant_results),"excels/canonical_pathway_results_significant_results_res1.csv")
write.csv(as.data.frame(canonical_pathway_results$top_10_padjusted),"excels/canonical_pathway_results_top_10_padjusted_res1.csv")
...
```{r}
# GTRD_gene_sets
GTRD_gene_sets_results <- GSEA_function(dataset_path = file.path(path,GTRD_gene_sets),namedlist = signif(metric,digits = 10),padj_filter = 0.05,title = "GTRD_gene_sets")

GTRD_gene_sets_results$filteredplot
datatable(GTRD_gene_sets_results$significant_results, extensions = 'Buttons', options = list(pageLength = 10, autoWidth = TRUE, dom = 'Bfrtip', scrollX = TRUE,buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
WikiPathways_results$plot

GTRD_gene_sets_results$top_10_highest_NES
GTRD_gene_sets_results$top_10_lowest_NES
```
...
plot_volcano <- function(labeled_results, label_threshold_log2FC = 1.3,label_threshold = 1.3, label_n = 5) {
  ...
  ggplot(labeled_results, aes(log2FoldChange, -log10(padj))) +
    ...
    geom_text(data = labeled_points, aes(label = Gene_name), nudge_x = 0.7, nudge_y = 2, size = 3) +
    xlab(expression("log2FoldChange")) + 
    ylab(expression("-log10(padj)")) +
    ggtitle("Differential Expression Analysis: KO/control vs KO AngII Analysis Volcano Plot") +
...
# final : creating hypothesis. looking in the literature for the effect sof angII . what these results mean. Cell replication? has this been seen before?
# normalized counts
# MONDAY presentation
# replicate reports
